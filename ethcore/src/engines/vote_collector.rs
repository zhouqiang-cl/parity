...
mod tests {
	...
	fn seal_retrieval() {
		...
		assert_eq!(signatures[0], collector.round_signatures(&propose_round, &bh.unwrap())[0]);
		assert_eq!(signatures[1..3].to_vec(), collector.round_signatures(&commit_round, &bh.unwrap()));
